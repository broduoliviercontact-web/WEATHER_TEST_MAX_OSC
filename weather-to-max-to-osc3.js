...
function sanitizeOscSegment(segment) {
...
function sendFlattenedOsc(prefix, value) {
  ...
  if (
    ...
    sendOscValue(prefix, value);
    return;
  }

  if (Array.isArray(value)) {
    value.forEach((item, index) => {
      sendFlattenedOsc(`${prefix}/${index}`, item);
    });
    return;
  }
  ...
  if (typeof value === 'object') {
    Object.entries(value).forEach(([key, nestedValue]) => {
      const safeKey = sanitizeOscSegment(key);
      sendFlattenedOsc(`${prefix}/${safeKey}`, nestedValue);
    });
  }
}
...
async function sendWeather() {
  try {
    ...
    // ---- Envois OSC ----
    sendFlattenedOsc('/weather/api', data);

    if (typeof temp === 'number') {
      oscClient.send('/weather/api/temp', temp);
    }

    if (typeof humidity === 'number') {
      oscClient.send('/weather/api/humidity', humidity);
    }

    if (typeof wind === 'number') {
      oscClient.send('/weather/api/wind', wind);
    }

    // Texte + code météo
    oscClient.send('/weather/api/main', main);
    oscClient.send('/weather/api/mainCode', mainCode);

    // Soleil : valeurs déjà "musicales"
    if (typeof sunriseNorm === 'number') {
      oscClient.send('/weather/api/sunrise_norm', sunriseNorm);       // 0–1
    }
    if (typeof sunsetNorm === 'number') {
      oscClient.send('/weather/api/sunset_norm', sunsetNorm);         // 0–1
    }
    if (typeof dayLengthNorm === 'number') {
      oscClient.send('/weather/api/day_length_norm', dayLengthNorm);  // 0–1
    }
    if (typeof dayProgress === 'number') {
      oscClient.send('/weather/api/day_progress', dayProgress);       // 0–1
    }
    if (typeof isDay === 'number') {
      oscClient.send('/weather/api/isDay', isDay);                    // 0 ou 1
    }
